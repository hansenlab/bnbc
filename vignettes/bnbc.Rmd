...
`r Biocpkg("bnbc")` uses the `ContactGroup` class to represent the set of contact matrices for a given set of interactions from a given of experiment. The class has 3 slots:
...
What is needed is essentially a list of contact matrices. We assume that the contact matrices are square, with no missing values and that they are not normalized (including for library depth).
